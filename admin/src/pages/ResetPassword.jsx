...
const ResetPassword = () => {
    ...
    const onSubmitOtp = async (e) => {
        ...
        setOtp(enteredOtp);
        try {
            const { data } = await axios.post(backendUrl + "/api/auth/otp-verification", { email, otp: enteredOtp })
            // data.success ? toast.success(data.message) : toast.error(data.message)
        ...
        <div className='flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-500 to-purple-400'>
            ...
            {!isEmailSent &&
                <form onSubmit={onSubmitEmail} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
                    <h1 className='text-white text-2xl font-semibold text-center mb-4'>Zresetuj hasło</h1>
                    <p className='text-center mb-6 text-indigo-300'>Wprowadź adres e-mail połączony z kontem.</p>
                    ...
                    <button className='w-full py-2.5 bg-gradient-to-r from-indigo-500 to-indigo-900 text-white rounded-full mt-3'>Prześlij</button>
                </form>}
            ...
            {isOtpSubmited && isEmailSent &&
                <form onSubmit={onSubmitNewPassword} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
                    <h1 className='text-white text-2xl font-semibold text-center mb-4'>Nowe hasło</h1>
                    <p className='text-center mb-6 text-indigo-300'>Wpisz nowe hasło poniżej</p>
                    <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
